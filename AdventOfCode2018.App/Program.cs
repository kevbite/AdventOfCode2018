...
|.||.|...|...|##..||....|#.|..#..##....|#.#|##..|.";
        ...
        {
            var input = Day16PuzzleInput.SampleData;

            var day16 = new Day16.PossibleOpCodesProvider();
            var stringReader = new StringReader(input);

            var puzzles = new List<Sample>();
            Sample sample = null;
            while ((sample=Sample.ReadNext(stringReader)) != null)
            {
                puzzles.Add(sample);
            }

            var possible = puzzles.Select(x => day16.GetPossibleOpCodes(x));
            var result1 = possible
                .Where(x => x.Length >= 3)
                .Count();

            var opCodeMap = new OpCodeMapBuilder(new PossibleOpCodesProvider()).BuildMap(puzzles);
            var programRunner = new ProgramRunner(new OperationsFactory(opCodeMap));
            var registers = programRunner.Run(Day16PuzzleInput.ProgramData);

            Console.WriteLine("Day 16 part 1 result: " + result1);
            Console.WriteLine("Day 16 part 2 result: " + registers[0]);
